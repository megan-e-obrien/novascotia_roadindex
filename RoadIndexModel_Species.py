...
# This version of the code is applied specifically for species at risk listed in Table 4 in the associated report.
...
# If calculating Road Index for a species listed below, this extent can be applied to limit the analysis to the species' core habitat. The raster will also be extracted by a mask generated by this polygon.
# However, if calculating the Road Index for several species listed, do NOT use and instead apply core habitat masking in post-processing.
...
# (5) Set desired species. The following is a list of all available species. Copy desired species into the active code.
# Species names must be exactly as written below. Species should be written between "" and separated by commas. See example below.
# Available species: ["marten", "grasslandbirds", "woodlandbirds", "turtles", "snakes", "bats", "moose", "bear", "lynx", "fisher", "otter", "beaver", "mink", "muskrat"]
species = ["bats", "marten", "turtles"]
...
# Loop through the list of selected species and apply the Road Index calculations sequentially.
for animal in species:
    print("Calculating for: " + animal)

    # Define avoidance thresholds
    avoidance_values = {
        "marten": [100, 100, 100, 100, 100],
        "grasslandbirds": [1200, 700, 400, 150, 1],
        "woodlandbirds": [810, 305, 150, 34, 1],
        "turtles": [500, 321, 179, 71, 1],
        "snakes": [1000, 730, 650, 257, 1],
        "bats": [1035, 665, 370, 148, 1],
        "moose": [1000, 750, 550, 400, 300],
        "bear": [1000, 574, 100, 73, 50],
        "lynx": [1000, 1000, 1000, 1000, 1000],
        "fisher": [1000, 1000, 1000, 1000, 1000],
        "otter": [1000, 1000, 1000, 1000, 1000],
        "beaver": [1000, 1000, 1000, 1000, 1000],
        "mink": [1000, 1000, 1000, 1000, 1000],
        "muskrat": [1000, 1000, 1000, 1000, 1000]
        }

    # Select the appropriate avoidance value from the list depending on the animal being evaluated in that loop
    avoidance = avoidance_values[animal]
...
def classify(featcode):
    ...
    def calculations(selectedLayer, value, avoidance_threshold):
        ...
        cellSize = 100
        searchRadius = avoidance_threshold
        ...
        sourceMaxAcc = avoidance_threshold

        # Run tool and save output as tif file
        ...
        featScore.save(out_workspace + animal + '_' + value + '_Score.tif')
    ...
    arcpy.CopyFeatures_management(NSTDB_NSRN, (out_workspace + animal + '_NSTDB_NSRN.shp'))
    ...
    # Save the altered layer as a shapefile to the code results folder
    arcpy.CopyFeatures_management(NSTDB_UT, (out_workspace + animal + '_NSTDB_UT.shp'))

    print("New field added to NSTDB_UT layer and calculated based on road feature codes.")

    # Reset input counter to 0 so that code can run for multiple species
    inputcount = 0
    ...
    for input in inputs:
        ...
        fc = 'Output_' + animal + '_' + inputname + '.shp'
        field = 'Road_Index'
        ...
        for value in uniqueValues:
            ...
            if value == 'Null':
                ...
                # The avoidance threshold is variable depending on the road type.
                # Based on the feature type being examined, av_val is set to the corresponding numeric value to retrieve the avoidance threshold from the species avoidance list.
                if value == "Highway":
                    av_val = 0
                elif value == "Paved":
                    av_val = 1
                    ...
                    print("No avoidance threshold could be found for the associated feature code")

                avoidance_threshold: int = avoidance[av_val]
                ...
                calculations(selectedLayer, value, avoidance_threshold)

        # Delete unnecessary files
        arcpy.management.Delete('Output_' + animal + '_' + inputname + '.shp')
    ...
    trailScore = (arcpy.sa.Raster('Output_' + animal + '_Trail_Score.tif'))
    utilityScore = (arcpy.sa.Raster('Output_' + animal + '_Utility_Score.tif'))
    allFeatureScore = (arcpy.sa.Raster('Output_' + animal + '_Merged_NSRN_UT_Score.tif'))
    gravelScore = (arcpy.sa.Raster('Output_' + animal + '_Gravel_Score.tif'))
    pavedRoadScore = (arcpy.sa.Raster('Output_' + animal + '_Paved_Score.tif'))
    highwayScore = (arcpy.sa.Raster('Output_' + animal + '_Highway_Score.tif'))
    ...
    # If a user-defined extent for species habitat has been set, extract the raster by a mask based on the extent.
    if userDefinedExtent:
        roadIndexFloat = ExtractByMask(roadIndexFloat, extent, "INSIDE")
    else:
        continue
    ...
    roadIndexFinal.save(out_workspace + animal + '_RoadIndex.tif')
    print("Road index raster saved")

    # Save road influence as tif file
    roadInfluenceFinal.save(out_workspace + animal + 'RoadInfluence.tif')
    ...
    # Delete feature type Road Index values for each animal. If you would like to calculate these values or only certain feature types, comment out the relevant statements.
    arcpy.management.Delete('Output_' + animal + '_Trail_Score.tif')
    arcpy.management.Delete('Output_' + animal + '_Utility_Score.tif')
    arcpy.management.Delete('Output_' + animal + '_Merged_NSRN_UT_Score.tif')
    arcpy.management.Delete('Output_' + animal + '_Gravel_Score.tif')
    arcpy.management.Delete('Output_' + animal + '_Paved_Score.tif')
    arcpy.management.Delete('Output_' + animal + '_Highway_Score.tif')
    arcpy.management.Delete('Output_' + animal + '_Merged_NSRN_UT.shp')
